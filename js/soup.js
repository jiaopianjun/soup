var v = new Vue({
  ...
  methods:{
    ...
    getonlist(){
      ...
      $.ajax({
        type: 'get',
        url: 'https://web.lieme.cn/api/jitang.php',
        dataType: 'jsonp',
        jsonp: 'callback',
        success:function(res) {
          if (res.status === 1) {
            _this.text = res.data[0].content
    ...
    local() {
      let _this  = this;
      _this.data = JSON.parse(localStorage.getItem('defaultList'))
      if (_this.data.length) {
        _this.text  = _this.data[Math.floor((Math.random()*_this.data.length))]
